...
import numpy
import traceback
from login.models import running_list
...
def readdata(request):
    if request.method == "POST":
        ...
        if by_name:
            ...
            # 模糊查询，包含内容就查询
        if new_search:                
            all_result = all_result.filter(Q(name__contains= new_search)|Q(user__contains= new_search) | Q(fun__contains=new_search) |
            ...
            # 获取首页的数据
        datas = all_result.loc[start:(start+length)]
            # 转为字典
            
        resp = [model_to_dict(obj) for obj in datas]
